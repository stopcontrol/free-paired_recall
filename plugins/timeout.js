    ...
    jsPsych['survey-text2'] = (function() {
        ...
        plugin.trial = function(display_element, block, trial, part) {
            ...
            for (var i = 0; i < trial.questions.length; i++) {
				...
				var df = 'Category '+ (block.trial_idx+1) +' / 8 :';
                // add question text
                $("#surveytext" + i).append('<p> ' + df + '</p>');
				$("#surveytext" + i).append('<p> <strong> <font size="5"  class="surveytext"  </font size>' + trial.questions[i] + '</strong></p>');
				$("#surveytext" + i).append('<p> List as many associations as you can (<i>and separate them by commas</i>):</p>');
                // add text box
                $("#surveytext" + i).append('<textarea autofocus id="message" name="textarea" style="width:400px;height:200px;"></textarea>');

                // add countr box
				$("#surveytext" + i).append('<p> <i></i></p>');
				$("#surveytext" + i).append(' <font size="3" color="black">Seconds left until the next category:</font>');
                $("#surveytext" + i).append(' <input  id="time" size="2"></input>');
